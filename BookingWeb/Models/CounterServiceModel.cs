...
namespace TGBookingWeb.Models
{
    public class CounterServiceModel
    {
        ...
        public CounterServiceModel(string frontURL, AirFare airfare, string pnr, string originCode, string destinationCode, string invoiceNo, bool isProd)
        {
            Url4Authorize = isProd ? "https://gateway.payatall.com/paynow" : "https://gateway.payatall.com/paynow";
            mid = "182";
            inv = invoiceNo;
            desp = "Air Ticket-" + pnr + "/" + originCode + destinationCode;
            //amt = (airfare.counterServiceFee + airfare.totalFare).ToString("N2").Replace(",", "");
            curr_type = "THB";
            serviceid = "01";
            language = Localize.GetLang().ToUpper();
